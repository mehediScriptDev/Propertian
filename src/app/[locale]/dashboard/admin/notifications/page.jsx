...
// import { showToast } from '@/components/Toast'; // Toast for feedback
...
//     // Optimistic UI Update & Adjust Count
//     setNotifications((prev) => {
...
export default function AdminNotificationsPage({ params }) {
  ...
  // Filter notifications (Updated Logic)
  const filteredNotifications = useMemo(() => {
    // If filter is 'unread', ensure only unread data is returned
    if (filterType === 'unread') {
      return notifications.filter((n) => n.unread === true);
    } 
    // If filter is 'read', return read notifications only
    else if (filterType === 'read') {
      return notifications.filter((n) => !n.unread);
    }
    // Default returns all
