...
export default function EventManagement({ params }) {
  ...
  // Computed stats for the overview cards
  const stats = useMemo(() => {
    const completed = events.filter((e) => (e.status || '').toString() === 'completed').length;
    const ongoing = events.filter((e) => (e.status || '').toString() === 'ongoing').length;
    const coming = events.filter((e) => (e.status || '').toString() === 'upcoming').length;
    const pending = events.filter((e) => (e.status || '').toString() === 'draft').length;
    return { completed, ongoing, coming, pending };
  }, [events]);
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
        <StatsCard title='Event Completed' value={stats.completed} icon={CheckCircle} />
        <StatsCard title='Ongoing Event' value={stats.ongoing} icon={PlayCircle} />
        <StatsCard title='Coming Event' value={stats.coming} icon={Clock} />
